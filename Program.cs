...
namespace CursorMovement
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            char[,] gameField = new char[4 + 51, 4 + 21]; // +4 because of where they start
            ...
            while (true)
            {
                ...
                {       // true: there is a key in keyboard buffer
                    ...
                    {
                        if (pdir == -1 && pX != 54 && gameField[pX + 1, pY] != '#') { Console.SetCursorPosition(pX + 1, pY); gameField[pX + 1, pY] = '+'; }
                        else if (pdir == 1 && pX != 4 && gameField[pX - 1, pY] != '#') { Console.SetCursorPosition(pX - 1, pY); gameField[pX - 1, pY] = '+'; }
                        else if (pdir == 2 && pY != 24 && gameField[pX, pY + 1] != '#') { Console.SetCursorPosition(pX, pY + 1); gameField[pX, pY + 1] = '+'; }
                        else if (pdir == -2 && pY != 4 && gameField[pX, pY - 1] != '#') { Console.SetCursorPosition(pX, pY - 1); gameField[pX, pY - 1] = '+'; }
                        else { success = false; }
                        if (success)
                        {
                    ...
                    {   // key and boundary control
                        ...
                        if (pX < 54 && gameField[pX + 1, pY] != '#')
                        {
                            Console.SetCursorPosition(pX, pY);           // delete X (old position)
                            Console.WriteLine(" ");
                            gameField[pX, pY] = '\0'; // so new random things will see its old place as null and be able to be placed there
                            pX++;
                            energy--;
                        }
                        pdir = 1;
                    ...
                    if (aliveZ) // enemy
                    { 
                        if (testZx < 54 && testZx > 4 && gameField[testZx - 1, testZy] != '#' && gameField[testZx + 1, testZy] != '#') 
                        {                                                     // prioritizing x
                            if (gameField[testZx - 1, testZy] != '#' && testZx > pX)
                            {
                                Console.SetCursorPosition(testZx, testZy);           // delete Z (old position)
                                Console.WriteLine(" "); 
                                gameField[testZx, testZy] = '\0';
                                testZx--;
                                // Note: Since enemies will already move each time unit, setcursor, writeline and nulling game field can be done
                                // below instead of 4 seperate times
                            }
                        ...
                        {
                            if (testZy > 4 && gameField[testZx, testZy - 1] != '#' && testZy > pY)
                            {
                                Console.SetCursorPosition(testZx, testZy);
                                Console.WriteLine(" ");
                                gameField[testZx, testZy] = '\0';
                            ...
                            else if (testZy < 24 && gameField[testZx, testZy + 1] != '#' && testZy < pY)
                            {
                                Console.SetCursorPosition(testZx, testZy);
                                Console.WriteLine(" ");
                                gameField[testZx, testZy] = '\0';
